...
-- v15 → v16: tabelas que existiam apenas em produção (criadas manualmente no
-- painel do Supabase), nunca versionadas. Cada rota que as usa já trata o erro
-- 42P01 (tabela inexistente) graciosamente, então a app nunca quebrou — mas uma
-- instalação nova a partir deste schema ficava sem estas 5 funcionalidades.
-- IF NOT EXISTS é idempotente: em produção (onde já existem) isto é um no-op.
...
-- v16 → v17: spin_off_tenant() — atomically creates a new tenant and moves
-- every per-tenant row into it. Used to separate the platform (default) tenant
...
-- (nothing half-moved). OPTIONAL tables (the 5 added in v15→v16, which may not
-- exist on every install) are wrapped in their own sub-block so a missing
-- table there doesn't abort the rest of the migration.
