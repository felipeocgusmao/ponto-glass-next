...
**Admin shell** — sidebar agrupada (OPERAÇÃO / PESSOAS / ANÁLISE), botão de colapsar com persistência em `localStorage`, topbar com breadcrumbs, busca **⌘K** que abre uma **Command Palette** (navegação + ações), e um **Settings Modal** centralizado para tema, idioma, palavra-passe e logout.
...
│   │       ├── TopBar            ← breadcrumbs + ⌘K + tema + notificações
